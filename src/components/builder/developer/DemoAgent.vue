<template>
  <v-card dark elevation="5">
    ...
    <v-card-actions>
      ...
      <v-btn v-if="agent.cellData" small icon :href="`http://localhost:4400${index + 1}?agentPubKey=${encodeURIComponent(agent.agentPubKey)}&${cells}&port=${port}`" target="_blank">
        <v-icon>
          mdi-application
        </v-icon>
      </v-btn>
...
export default {
  ...
  computed: {
    ...mapState('builderDeveloper', ['hcClient', 'dnaPaths']),
    cells () {
      let cellIds = ''
    ...
    port () {
      return process.env.VUE_APP_HOLOCHAIN_APP_INTERFACE_DOCKER_PORT
    }
  },
  methods: {
    ...mapActions('builderDeveloper', ['generateAgentKey', 'installDna']),
    openAgentDetail () {
