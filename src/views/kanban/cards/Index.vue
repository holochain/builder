        ...
        >
          <split-area :size="22">
            <v-toolbar dense dark>
              ...
              <v-spacer></v-spacer>
              <v-icon @click="$store.dispatch('kanban/fetchCards')" color="primary" dark class="pr-1">
                mdi-refresh
              </v-icon>
...
export default {
  ...
  components: {
    VImageInput,
    Column: () => import('@/components/kanban/Column.vue'),
    CardTree: () => import('@/components/kanban/CardTree.vue'),
    ConfirmActionDialog: () =>
  ...
  computed: {
    ...mapState('kanban', ['treeItems', 'selectedColumn', 'cards']),
    columns () {
      return this.cards
  ...
  methods: {
    ...mapActions('kanban', ['saveCard', 'deleteCard']),
    ...mapMutations('kanban', ['setSelectedColumn']),
    addRootColumn () {
